demo = {
    ...
    initChartPageCharts: function(reportData) {
        let monthLabels         = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
        let profit_report       = new Array(12).fill(0);
        let quantity_report     = new Array(12).fill(0);

        for (let i=0; i < 12; i++){
            let dd = reportData.find(d => d.month === i);

            if(dd){
                let index = dd.month;
                profit_report[index]        += dd.profit;
                quantity_report[index]      += dd.quantity;
            }
        }

        console.log({profit_report, quantity_report})
    ...
    showNotificationSuccess: function(from, align, message) {
        color = 'success';
